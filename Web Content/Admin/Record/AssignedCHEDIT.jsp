...
<body>
		<%
			 int id=Integer.parseInt(request.getParameter("CH_AssignedID"));
			 dbtbch_assigned c=ch_assignedDAO.getCHRecord(id);
		%>
    					  ...
    					  <div class="modal-content">
					        ...
					        <label>Chef</label>
					        	<input type="hidden" name="CH_AssignedID" value="<%=id%>">
									<input type="hidden" name="CHAS_ChefID" value="<%=c.getCHAS_ChefID()%>" id='CH_ID'>
					        <%
								    	try{
								    		Connection con=DbCon.getCon();
								    		Statement stmt = con.createStatement();
								    		ResultSet rs = stmt.executeQuery("SELECT chef_ID,CONCAT(Lastname,', ',FirstName,' ',MiddleName) FROM chef_record WHERE chef_ID=" + c.getCHAS_ChefID());					                        	
								    %>
							...
							<label>Date Started</label>
								...
								<br/>
									...
									<input type="hidden" id='P_ID' name="AssignedPatient" value="<%=c.getAssignedPatientID()%>"/>
					        		
					        		<%
								    	try{
								    		Connection con=DbCon.getCon();
								    		Statement stmt = con.createStatement();
								    		ResultSet rs = stmt.executeQuery("SELECT Patient_ID,CONCAT(Surname,', ',FirstName,' ',MiddleName) FROM client_record WHERE Patient_ID=" + c.getAssignedPatientID());					                        	
								    %>
